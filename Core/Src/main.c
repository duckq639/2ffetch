...
{
  if (HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &DJCAN_RxFrame.CAN_RxHeader, DJCAN_RxFrame.Data) == HAL_OK)
  {
    if (hcan == DJCAN_RxFrame.hcan)
    {
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if (htim->Instance == TIM8) // 触发频率1000hz
  {
    DJMotor_Func();
    if (++timetick >= 10)
    {
